package com.fengdis.common.component.rpc.elasticsearch;
...
public class TransportClientUtils {
    ...
    private static volatile TransportClientUtils transportClientUtils = null;

    /**
     * 这里使用饿汉单例模式创建TransportClient
     */
    public static TransportClientUtils getInstance() {
        if (transportClientUtils == null){
            synchronized (TransportClientUtils.class){
                if (transportClientUtils == null){
                    transportClientUtils = new TransportClientUtils();
                }
            }
        }
        return transportClientUtils;
    }
    ...
    private TransportClientUtils() {
        try {
            Settings settings = Settings.builder()
                    ...
                    .put("cluster.name", PropertiesUtils.getString("elasticsearch.cluster-name","elasticsearch")).build();
            client = new PreBuiltTransportClient(settings)
                    .addTransportAddress(new TransportAddress(
                            new InetSocketAddress(PropertiesUtils.getString("elasticsearch.cluster-ip","127.0.0.1"),
                                    Integer.valueOf(PropertiesUtils.getString("elasticsearch.cluster-port","9300")))));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
