    ...
    private RestHighLevelClient getClient(){
        try {
            client = new RestHighLevelClient(
                    RestClient.builder(
                            new HttpHost(PropertiesUtils.getString("elasticsearch.cluster-ip","127.0.0.1"),
                                    Integer.valueOf(PropertiesUtils.getString("elasticsearch.cluster-port","9300")), "http")
                    )
            );
        } catch (Exception e) {
            e.printStackTrace();
